...
export function generateResumeMarkdown(data) {
  ...
  if (data.contactInfo.linkedin) {
    const displayUrl = data.contactInfo.linkedin.replace(/^https?:\/\/(www\.)?/, '');
    contact.push(`💼 [${displayUrl}](${data.contactInfo.linkedin})`);
  }
  if (data.contactInfo.twitter) {
    const displayUrl = data.contactInfo.twitter.replace(/^https?:\/\/(www\.)?/, '');
    contact.push(`🐦 [${displayUrl}](${data.contactInfo.twitter})`);
  }
